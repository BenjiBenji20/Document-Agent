...
class GeminiAgents(BaseAgent):
    ...
    async def extract_single_document_schema(self, metadata: dict) -> dict:
        ...
        if not mime_type:
            return self._failed_response(file_id, file_name, "Missing mime_type.")
        ...
        try:
            ...
            response = await self.client.aio.models.generate_content(
                ...
                contents=[
                    uploaded_file,  # Gemini File API reference — Gemini fetches it directly
                    "Identify and extract the document structural fields."
                ],
                config=types.GenerateContentConfig(
                    system_instruction=self.WORKER_EXTRACT_SCHEMAS_PROMPT,
                    response_mime_type="application/json",
                    response_schema=AgentExtractedDocumentMetadata, # clean pydantic model. No examples argument, Annotations obj
                    temperature=0.1
                )
            )
            ...
            return self._failed_response(file_id, file_name, str(e))
